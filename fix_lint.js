...
const replaceMotion = (fileName) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    // We only want to remove `motion` from `import { motion, ... } from 'framer-motion'`
    // If it's `import { motion } from 'framer-motion';` we can leave it or remove it entirely.
    // Let's replace 'motion,' with '' and '{ motion }' with '{}' temporarily.
    // Or just append '// eslint-disable-next-line no-unused-vars' before the import.
    content = content.replace(/import\s+\{\s*motion[\s,]*([^}]*)\}\s+from\s+['"]framer-motion['"];/g, 
                             "import { $1 } from 'framer-motion';");
    // if it became empty import {  } from 'framer-motion'; just leave it, it's fine.
    
    // There are cases where it is `import { motion } from` -> `import {  } from`
...
// Now fix CertificationsSection.jsx by truncating to </section> and appending modals cleanly
